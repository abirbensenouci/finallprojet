...
const Rv = () => {
  ...
  const handleAppointmentSubmit = () => {
    if (selectedDate && selectedTime) {
      ...
      fetch(`http://localhost:8081/rendez_vous/${code_barrau}/${id_client}`, {
        method: 'POST',
        headers: {
        ...
        .then(response => {
          if (response.ok) {
            ...
            console.log("Err444eur lors de l'enregistrement du rendez-vous");
          }
        })
        .catch(error => {
          console.error(error);
  ...
  return (
    <><Nav /><div className="containerrv">
          <h2 className='h2rv'>Rendez-Vous</h2>
